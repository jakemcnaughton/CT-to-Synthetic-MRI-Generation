...
class TransformerUNet(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # Decoder
        x = self.upconv4(x)
        x = self.x4_1(concatenate([x, out4], axis=-1))
        x = self.x4_2(x)
        x = self.upconv3(x)
        x = self.x3_1(concatenate([x, out3], axis=-1))
        x = self.x3_2(x)
        x = self.upconv2(x)
        x = self.x2_1(concatenate([x, out2], axis=-1))
        x = self.x2_2(x)
        x = self.upconv1(x)
        x = self.x1_1(concatenate([x, out1], axis=-1))
        x = self.x1_2(x)
...
def unet():
  ...
  model.compile(optimizer = Adam(learning_rate = 5e-5, clipnorm=1), loss = 'MeanAbsoluteError', metrics = ['MeanSquaredError',PSNR, SSIM])
