...
class PatchSequence(Sequence):
    ...
    def __init__(self, x_lists, y_lists, batch_size,
                 patch_size, stride, volumes_to_analyse=125, first_vol=0, unet=True,
    ...
    def get_linear_gradients(self, dims):
        ...
        grad_i = np.zeros(dims, dtype=float)
        grad_j = np.zeros(dims, dtype=float)
        grad_k = np.zeros(dims, dtype=float)
    ...
    def __getitem__(self, batch):
        ...
        batch_y = np.concatenate(batch_y[:, :, :, :, :self.no_of_xs], axis=0)
        x_1 = x_1.reshape(self.batch_size,
                          self.patch_size,
                          self.patch_size,
                          self.patch_size,
